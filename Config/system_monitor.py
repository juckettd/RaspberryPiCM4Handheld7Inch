...
def increase():
	...
	if button_misc.is_pressed:
		...
		subprocess.run("amixer -q -M set PCM 3%+", shell=True)
...
def shutdown_pi():
	...
	subprocess.run("/usr/local/bin/multi_switch.sh --es-poweroff", shell=True)
...
button_shutdown.when_released = shutdown_pi
