...
screenCheck() {
	...
	if [[ "${DISPLAYS}" == *"HDMI"* ]]; then
		# Use ignore_lcd in the /boot/config.txt as the indicator that HDMI is setup
		if [ -e $CONFIG ] && grep -q -E "^#ignore_lcd=1$" $CONFIG; then
			# Setup HDMI

			sudo sed -i "s|^#hdmi_group|hdmi_group|" $CONFIG
			sudo sed -i "s|^#hdmi_mode|hdmi_mode|" $CONFIG
			sudo sed -i "s|^#ignore_lcd=1$|ignore_lcd=1|" $CONFIG

			if grep =q -E "^hdmi_group=0$" $CONFIG || grep -q -E "^hdmi_group=1$" $CONFIG; then
