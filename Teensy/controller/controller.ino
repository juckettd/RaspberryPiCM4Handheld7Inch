...
void setup() {
  ...
  XInput.setJoystickRange(275, 725);
}
...
void loop() {
  ...
  {
    bigRumbleCounter = 0;
    bigRumbleSet = bigRumble / 4 + 60;
  }
  else
  {
    if (bigRumbleCounter > 4)
      bigRumbleSet = 0;
    else
  ...
  if (smallRumble > 0)
  {
    ...
    smallRumbleSet = smallRumble / 4 + 60;
  }
  else
  {
    if (smallRumbleCounter > 4)
      smallRumbleSet = 0;
    else
  ...
  if(bigRumble < 30)
    bigRumble = 0;
  else if (bigRumble < 105)
    bigRumble = 45;
  else
    bigRumble = bigRumble / 3 + 10;

  if(smallRumble < 30)
    smallRumble = 0;
  else if (smallRumble < 105)
    smallRumble = 45;
  else
    smallRumble = smallRumble / 3 + 10;
  */
